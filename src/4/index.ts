...
function arrayIncludesFirstOrLastDigits(arr: Array<number>, first: number, last: number): boolean {
    if (arr.includes(first)) return true;
    if (arr.includes(last)) return true;
    return false;
}
...
elfTasks.map((tasks) => {
    ...
    if (
        arrayIncludesFirstOrLastDigits(
            elfSortedTasks[0],
            elfSortedTasks[1][0],
            elfSortedTasks[1][elfSortedTasks[1].length - 1]
        )
